...
def load_and_preprocess(df):
    ...
    if 'registration_date' in df.columns and 'purchase_date' in df.columns and 'last_login_history' in df.columns:
        ...
        if not np.issubdtype(df['purchase_date'].dtype, np.datetime64):
            df['purchase_date'] = cudf.to_datetime(df['purchase_date'])

        if not np.issubdtype(df['last_login_history'].dtype, np.datetime64):
            df['last_login_history'] = cudf.to_datetime(df['last_login_history'])
    ...
    df_final['recency_days'] = (current_time - cudf.to_datetime(df_final['last_purchase'])).dt.days
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    df = load_and_preprocess(df).to_pandas()
    df.to_csv(f'{output_dir}/part_{args.idx}.csv', index=False)
