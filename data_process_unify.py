...
def load_and_preprocess(df):
    ...
    if 'registration_date' in df.columns:
        ...
        if not np.issubdtype(df['registration_date'].dtype, np.datetime64):
            df['registration_date'] = cudf.to_datetime(df['registration_date'])
    ...
    if 'registration_date' in df.columns and 'timestamp' in df.columns:
        ...
        # 确保注册时间早于第一次消费时间
        time_mask = (df['registration_date'] <= df['timestamp']).to_pandas()
        mask = mask & time_mask  # 更新掩码
        # df = df[time_mask]

    # 处理异常值
    pre_filter_count = len(df[time_mask])
    age_mask = df['age'].between(18, 100).to_pandas()
    income_mask = (df['income'] > 0).to_pandas()
    credit_mask = (df['credit_score'] > 0).to_pandas()
    ...
    # 过滤掉没有购物记录或者记录为空的用户
    df = df[purchase_features['items_count'] > 0]
    purchase_features = purchase_features[purchase_features['items_count'] > 0]
    ...
    temp_df = df[['user_id', 'category_distribution']].to_pandas()
...
if __name__ == "__main__":
    ...
    for i, file in enumerate(parquet_files):
        ...
        df.to_csv(f'{output_dir}/data_part_{i}.csv', index=False)
